...
btn.addEventListener("click", (e) => {
    ...
    axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=${APIKey}&units=metric&lang=ar`)
    .then(res => {
        displayWeather(res.data);
    })
    .catch(error => {
        problem.textContent  = "عذراً... حدث خطاء ما";
...
function displayWeather(data) {
    ...
    const weatherHtml = `
        <div class ="fw-bold p-3 mb-1 rounded-4 w-75" style="color: #0E172A; border: 5px solid #0E172A;">
             <p>المدينة : ${data.name}</p><div style="border-radius: 5px; height: 2px; width: 100%; background-color: #0E172A;"></div>
            ...
            <p class = "mt-2">الرطوبة :  ${data.main.humidity}%</p>
        </div>
    `;
